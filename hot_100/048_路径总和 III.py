...
class Solution:
    def pathSum(self, root: Optional[TreeNode], targetSum: int) -> int:
        ...
        self.pathSum_help(root, res)      

        print(f"res: {self.res}")
        count = 0
        for cur_num in self.res:
            if cur_num == targetSum:
                count += 1
        return count
    
    def pathSum_help(self, root: Optional[TreeNode], res) -> int:
        if not root:
            return
        new_res = [root.val]
        for cur_num in res:
            new_res.append(root.val + cur_num)
        # self.res.extend(new_res)
        self.res.append(new_res)
        
        self.pathSum_help(root.left, new_res)
        self.pathSum_help(root.right, new_res)
...
if __name__ == "__main__":
    ...
    solution = Solution()
    new_root = solution.pathSum(root, targetSum)
