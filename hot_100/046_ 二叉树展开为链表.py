...
class Solution:
    def flatten(self, root: Optional[TreeNode]) -> None:
        levels = []
        self.preorder_help(root, levels)
        for i in range(len(levels) - 1):
            levels[i].right = levels[i+1]
            levels[i].left = None
        return root
        
    def preorder_help(self, root:TreeNode, res):          
        if not root:
            return
        res.append(root)
        self.preorder_help(root.left, res)
        self.preorder_help(root.right, res)
...
思路:
    思路. 
    1. 首先利用先序遍历得到节点列表
    2. 改变左右指针指向，左指针为空，右指针为当前节点下一个节点
"""
...
if __name__ == "__main__":
    ...
    solution = Solution()
    new_root = solution.flatten(root)
